...
static void add_material(vk::Pipeline pipeline, vk::PipelineLayout layout, std::string const &name, std::unordered_map<std::string, Material> &materials, std::list<vk::PipelineLayout> &pipeline_layouts) {
...
public:
    ...
    build_materials(vk::Device device, vk::RenderPass renderpass, vk::DescriptorSetLayout descriptor_set_layout, vk::DescriptorSetLayout lighting_descriptor_set_layout,
        ...
        auto base_shader_dir = std::string{get_asset_dir()} + "/shaders/";
        ...
        VertexInputDescription vertex_input_description_skinning = VertexBone::get_vertex_description();
        ...
        VertexInputDescription empty_vertex_input{};
...
private:
    static vk::Result load_shader_module(const std::string& file_path, vk::Device device, vk::ShaderModule *out_shader_module) {
