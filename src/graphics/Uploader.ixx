...
struct MeshUpload {
    ...
    vk::Fence fence;
    std::string name;
    lime::AllocatedBuffer vertex_buffer;
    lime::AllocatedBuffer index_buffer;
    vk::CommandBuffer command_buffer;
};

struct TextureUpload {
    lime::GPUTexture texture;
    vk::Fence fence;
    std::string name;
    lime::AllocatedBuffer buffer;
    vk::CommandBuffer command_buffer;
};
...
export namespace lime {
    ...
    public:
        ...
        ~Uploader() {
            utils::assert_true(!m_upload_thread.keep_running);
        };
        ...
        void poll_uploads(std::unordered_map<std::string, GPUMesh> &meshes,
            ...
            for (auto it = m_upload_thread.uploaded_meshes.begin(); it != m_upload_thread.uploaded_meshes.end();) {
                ...
                utils::assert_true(status != vk::Result::eErrorDeviceLost);
            }
    ...
    private:
        ...
        MeshUpload upload_mesh(std::string_view name) {
            ...
            auto mesh = mesh::load_from_gltf(name, false);
            ...
            if (skinning) {
                auto &mesh_tmp = std::get<mesh::MeshBone>(mesh);
                vertex_buffer_size = mesh_tmp.vertices.size() * sizeof(mesh::VertexBone);
                ...
                auto &mesh_tmp = std::get<mesh::Mesh>(mesh);
                vertex_buffer_size = mesh_tmp.vertices.size() * sizeof(mesh::Vertex);
                index_buffer_size = mesh_tmp.index_buffer.size() * sizeof(uint32_t);
                index_count = (uint32_t) mesh_tmp.index_buffer.size();
                vertex_data = mesh_tmp.vertices.data();
                index_data = mesh_tmp.index_buffer.data();
            ...
            auto result_vma = vmaCreateBuffer(m_allocator,
                                              ...
                                              nullptr);
            utils::vk_assert(std::bit_cast<vk::Result>(result_vma));
        ...
        TextureUpload upload_image(std::string_view name) {
            ...
            std::string file = std::string{utils::get_asset_dir()};
            file += name;
            file += ".png";
            ...
            utils::assert_true(load_image_from_file(file, upload.texture.image, upload.fence, upload.command_buffer, upload.buffer));
            ...
            utils::assert_true(result == vk::Result::eSuccess);
            return upload;
        };
        ...
        bool load_image_from_file(std::string const &file, AllocatedImage &new_image, vk::Fence fence,
            ...
            utils::vk_assert(std::bit_cast<vk::Result>(result));
